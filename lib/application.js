...
class Application {
  ...
  async readCardsFromControllers(controllerIds) {
    ...
    for (const ctrl of targetControllers) {
      try {
        ...
        const cards = await this.wrapperAPI.getCards(lCTX, ctrl.serial);

        if (Array.isArray(cards)) {
          for (const card of cards) {
            const cardNumber = parseInt(card.cardNumber || card.card, 10);
            if (!isNaN(cardNumber)) {
  ...
  async applyMigration(cardsFromControllers, mode = "overwrite") {
    ...
    for (const card of cardsFromControllers || []) {
      cardsFromMap.set(card.cardNumber, card);
    }
