 ...
 * @returns {it is Record<string, any>} True if the variable is a plain object
 */
 ...
 *
 * @param {any} it The variable to test
 * @returns {it is any[]} True if the variable is an array
 */
function isArray(it) {
...
async function translateGoogle(text, targetLang) {
  try {
    ...
    const response = await axios({
      url,
      proxy: { host: "sia-lb.telekom.de", port: 8080 },
      timeout: 15000,
    });
